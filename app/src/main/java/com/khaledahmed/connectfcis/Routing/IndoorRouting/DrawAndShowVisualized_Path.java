...
/**
 * get the path and draw on image and show it.// in details:-
 * get selected src and dest input form visualizedRoutingActivity, then:-
 * build a graph of rooms in (Applied on first floor), then
 * send src and dest to the Graph Algorithm, then get the path, then
 * send 2 pixels a time(of all pixels in the path) to draw on image, then get the image with the path drawn to show it .
 */
...
// call the func here /////////////////
    ...
    public Bitmap drawFunc() {
        ...
        for (int f = 0; f < Rooms_pixels.size() - 1; f++) {
            srcPixel = Rooms_pixels.get(f);
            destPixel = Rooms_pixels.get(f + 1);
